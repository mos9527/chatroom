...
parser.add_argument(
    "--no-interact", action="store_true", help="Disable interactive CLI"
)
...
class File:
    def __init__(
        self, temp_file_path, file_size, file_name, file_type, object_type, file_key=""
    ) -> None:
...
class ChatSession(WebsocketSession):
    ...
    def cln_msg(self, msg):
        return ChatSession.msg(
            self.name,
            msg=(
                msg
                if self.unblock_state
                else "<pre>%s</pre>" % msg.replace("<", "&lt;").replace(">", "&gt;")
            ),
        )
    ...
    def ip(self):
        return self.request.headers.get(
            "x-forwarded-for",
            f"{self.request.client_address[0]}:{self.request.client_address[1]}",
        )
    ...
    def im(self, name):
        if name in ChatSession.username_blacklist:
            self.send_srv_msg(
                msg="<error>Invalid username %s - Username was rerserved</error>" % name
            )
        elif name in self.sess_users:
            self.send_srv_msg(
                msg="<error>Invalid username %s - Username was taken</error>" % name
            )
        else:
            self.name = name
    ...
    def unblock(self, opt=None):
        ...
        self.send_srv_msg(
            msg="<success>Turned %s HTML Tags.</success>"
            % ["on", "off"][not self.unblock_state]
        )
        return True
    ...
    def onOpen(self, request: Request = None, content=None):
        ...
        if self.get("banned-until", None) and self.get("banned-until") > time.time():
            self.send_srv_msg(
                msg="<error>You're still banned from this server : %s (for %ss)</error>"
                % (
        ...
        self.send_srv_msg(
            type="announce",
            msg=[
                "<b>Help: </b><code>!%s</code>: %s" % item
                for item in self.command_whitelist.items()
            ],
        )
        self.users()
        ...
        self["online"] = True
        self.logger.info(
            "%s : Connected via %s" % (self, self.request.useragent_string)
        )
    ...
    def __repr__(self) -> str:
        return "<ChatSession IP=%s SessionId=%s Name=%s>" % (
            self.ip,
            self.session_id,
            self.name,
        )
...
class FileSession(Session):
    ...
    def _file_upload(self, request: Request, content):
        ...
        if not self.request.headers.get("Content-Length"):
            return self.request.send_error(
                http.HTTPStatus.BAD_REQUEST, "Content-Length unknown"
            )
        if not self.request.headers.get("Content-Disposition"):
            return self.request.send_error(
                http.HTTPStatus.BAD_REQUEST, "Missing Content-Disposition header"
            )
        ...
        file_type, object_type = (
            self.request.headers.get("Content-Type") or "text/plain",
            self.request.headers.get("X-Object-Type") or "file",
        )
        disposition = parse_qs(disposition)
        filename = disposition["filename"][0]
        file_key = self.new_uid
        file = File(
            os.path.join(TEMP_PATH, file_key),
            length,
            filename,
            file_type,
            object_type,
        ...
        self.logger.info(
            (self.chat_session.get("name") or self.session_id)
            + " : Uploading "
            + filename
            + " -> "
            + file.temp_file_path
        ...
        boardcast(
            ChatSession.msg(
                sender=self.chat_session.get("name") or self.session_id,
                type=object_type,
                msg=file_key,
            )
        )
    ...
    temp_img_default = base64.b64decode(
    ...
    def _file_get(self, request: Request, content):
        ...
        self.logger.info(
            (self_name or self.request.useragent_string + "??")
            + " : Downloading "
            + file.file_name
        )
        ...
        if "thumb" in request.query:
            ...
            if not isfile(temp_img_path):
                ...
                try:
                    ...
                    # failed to parse
                    return WriteContentToRequest(
                        request,
                        file.temp_file_path,
                        partial_acknowledge=True,
            ...
            if not self_name in file.downloader:
                ...
                boardcast(
                    ChatSession.srv_msg(
                        msg={"file": file.dict(), "user": self_name},
                        type="filedownload",
                    )
                )
            return WriteContentToRequest(
...
if __name__ == "__main__":
    ...
    def lsu(filter=None):
        ...
        for session in filter or get_sessions():
            print(
                "[%s]" % session["ID"],
                ",".join(["%s=%s" % (k, v) for k, v in session.items()]),
            )
            count += 1
        print("- Total %s" % count)
    ...
    def kick(filter, reason=""):
        for session in filter:
            connection = get_connection_by_id(session["ID"])
            connection.send_srv_msg(
                msg="You are kicked from the server : %s"
    ...
    def ban(filter, duration=30, reason=""):
        for session in filter:
            connection = get_connection_by_id(session["ID"])
            connection.send_srv_msg(
                msg="You are banned from the server : %s"
                % (reason or "(no reason given)")
            )
    ...
    * Selections are made by File Filters (see above)
    ...
    def save(to="chat.json"):
        return open(to, encoding="utf-8", mode="w").write(
            json.dumps(boardcasts, ensure_ascii=False, indent=4)
        )
    ...
    if not args.no_interact:
        interact(
            banner="* Console is now ready (Press Ctrl+D to exit).", local=locals()
        )
    else:
        tServer.join()
